/*
 * struct _GDebugKey
 * Defined in file gutils.h @ line 151
 */
...
mrb_GLib_GDebugKey_set_key(mrb_state* mrb, mrb_value self) {
  ...
  const gchar * native_field = TODO_mruby_unbox_gchar_PTR(ruby_field);
...
void mrb_GLib_GDebugKey_init(mrb_state* mrb) {
  RClass* GDebugKey_class = mrb_define_class_under(mrb, GLib_module(mrb), "GDebugKey", mrb->object_class);
