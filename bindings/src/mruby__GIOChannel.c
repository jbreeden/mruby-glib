...
mrb_GLib_GIOChannel_get_encoding(mrb_state* mrb, mrb_value self) {
  ...
  mrb_value encoding = mrb_str_new(mrb, native_encoding, native_encoding_len);
...
mrb_GLib_GIOChannel_set_encoding(mrb_state* mrb, mrb_value self) {
  struct _GIOChannel * native_self = mruby_unbox__GIOChannel(self);
  char * encoding = NULL;
  mrb_int encoding_len = 0;

  mrb_get_args(mrb, "s!", &encoding, &encoding_len);
...
mrb_GLib_GIOChannel_get_line_term(mrb_state* mrb, mrb_value self) {
  ...
  mrb_value line_term = mrb_str_new(mrb, native_line_term, native_line_term_len);
...
mrb_GLib_GIOChannel_set_line_term(mrb_state* mrb, mrb_value self) {
  struct _GIOChannel * native_self = mruby_unbox__GIOChannel(self);
  char * line_term = NULL;
  mrb_int line_term_len = 0;

  mrb_get_args(mrb, "s!", &line_term, &line_term_len);
